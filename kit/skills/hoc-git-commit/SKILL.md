...
- **`Install` and `Uninstall` name the dependency, not the file that records it.** `Install
  ...
  installed is `Update`.
- **A `package-lock.json` regenerated by a command takes that command as its subject.** The
  form is `Update package-lock.json after npm <command>` — `after npm install`, `after npm
  update`, `after npm audit fix`, naming whatever produced the file.
  ...
  A lock file is not a decision of its own. It is the recorded outcome of one, and the command
  is the only thing about it a reader cannot recover from the diff — the versions are right
