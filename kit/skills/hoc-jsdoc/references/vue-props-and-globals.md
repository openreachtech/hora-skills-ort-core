...
`RequiredExcept`, `OptionalExcept`, `NullableExcept` are declared in `types/global.d.ts` under `declare global`, so they're used **unqualified** in JSDoc with no `@import`. Same for `schema.graphql.*` ([[hof-graphql]]), `furo.*`, and `GraphqlType.*` ([[hof-nuxt]]).
